...
dataset_path = 'digit_local_data.pkl'
test_path = 'digit_test_data.pkl'
svhn_size = 800
size = 250
u_size = 140
...
def prepare_server(da=False):
    ...
    if da:
        ...
        dataset_s = dataset.SVHN(root =dataroot, split ='test', download = True)
    data_u = [data[0].resize((32,32)).convert(mode='RGB') for idx, data in enumerate(dataset_u)]
    data_m = [data[0].resize((32,32)).convert(mode='RGB') for idx, data in enumerate(dataset_m)]
    data_s = [data[0].resize((32,32)).convert(mode='RGB') for idx, data in enumerate(dataset_s)]
    label_s = np.array(dataset_s.labels)
    label_u = np.asarray(dataset_u.targets)
    label_m = np.asarray(dataset_m.targets)
    ...
    index =np.array( np.arange(len(label_s)))
    #np.random.shuffle(index) #No shuffling, or the cache result is not correct
    keep_len = int(len(index) * ratio)
    index = index[:keep_len]
    for i in index:
        global_data.append(data_s[i])
        global_label.append(label_s[i])
