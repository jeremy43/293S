...
from dataset.cov_dataset import Custom_Dataset, Server_Dataset
from utils.sampling import mnist_iid, mnist_noniid, cifar_iid
#from utils.options import args_parser
from utils.cov_options import args_parser
...
if __name__ == '__main__':
    ...
    elif args.model =='cnn' and args.dataset == 'cov':
        net_glob = CNNCifar(args=args).to(args.device)
    ...
    for iter in range(args.epochs):
        ...
        LR= args.lr / math.pow((1 + 10 * (iter*nb_batch - 1) / (args.epochs*nb_batch)), 0.75)
        LR = args.lr
        print('curent lr', LR)
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)
        for idx in idxs_users:
            if args.da:
                local = DA_LocalUpdate(args=args, dataset=dataset_train,tgt_dataset = dataset_global, idxs=dict_users[idx], iter =LR)
            else:
